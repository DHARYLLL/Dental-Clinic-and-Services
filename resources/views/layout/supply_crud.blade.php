...
<div class="row m-2">
   <div class="card">
      <div class="card-body p-1">
            ...
            <ul class="list-group mb-1 ">
               <li class="list-group-item d-flex align-items-center justify-content-between border border-2">
                  <div class="d-flex justify-content-between align-items-center w-100 px-2">
                     <div class="">
                        <span><strong>Name: </strong>{{$supply->Name}}</span><br>
                        <span><strong>Quantity: </strong>{{$supply->Quantity}}</span><br>
                        <span><strong>Description: </strong>{{$supply->Description}}</span>
                     </div>

                     <div class="d-flex gap-3">
                        <div>
                           <button class="btn btn-dark w-100 px-2 py-1" data-bs-toggle="modal"
                              data-bs-target="#editSuppliesModal{{$supply->id}}"><i
         ...
         <div class="modal fade" id="editSuppliesModal{{$supply->id}}" tabindex="-1"
            aria-labelledby="editSuppliesModalLabel{{$supply->id}}" aria-hidden="true">
                  ...
                  <div class="modal-body mt-3">
                     <form action="{{route('supply.update', ['supply' => $supply])}}" method="POST">
                        ...
                        <div class="row mb-3 gap-2">
                           <div class="col">
                              <input style="background-color: #d9d9d9" type="text" id="SupplyName" name="Name"
                                 class="form-control p-2" value="{{$supply->Name}}">
                           </div>
                           <div class="col">
                              <input style="background-color: #d9d9d9" type="number" id="SupplyQuantity" name="Quantity"
                                 min="1" class="form-control p-2" value="{{$supply->Quantity}}">
                           </div>
                        </div>

                        <div class="row mb-3">
                           <textarea style="background-color: #d9d9d9" name="Description" id="SupplyDescription"
                              class="form-control pb-5">{{$supply->Description}}</textarea>
                        </div>
...
<div class="modal fade" id="addSuppliesModal" tabindex="-1" aria-labelledby="addSuppliesModalLabel" aria-hidden="true">
   <div class="modal-dialog modal-dialog-centered" style="max-width: 500px;">
      ...
      <div class="modal-content">
         ...
         <div class="modal-body mt-3">
            <form action="{{route('staffs.supply.store')}}" method="POST">
               @csrf
               @method('POST')
               <div class="row mb-3 gap-2">
                  <div class="col">
                     <input style="background-color: #d9d9d9" type="text" id="SupplyName" name="Name"
                        placeholder="Supply Name" class="form-control p-2">
                  </div>
                  <div class="col">
                     <input style="background-color: #d9d9d9" type="number" id="SupplyQuantity" name="Quantity"
                        placeholder="Quantity" min="1" class="form-control p-2">
                  </div>
               </div>
